...
import {
  addProductSuccess,
  clearProductSearchState, editProductSuccess, getProductsSuccess,
  ...
  switch (type) {
    ...
    case ADD_PRODUCT: {
      try {
        ...
        const key = yield call(firebase.generateProductKey);
        const downloadURL = yield call(firebase.storeImage, key, 'products', payload.image);
        const image = { id: key, url: downloadURL };
        let images = [];

        if (imageCollection.length !== 0) {
          const imageKeys = yield all(imageCollection.map(() => firebase.generateProductKey));
    ...
    case EDIT_PRODUCT: {
      try {
        ...
        if (imageCollection.length > 1) {
          ...
          const imageKeys = yield all(newUploads.map(() => firebase.generateProductKey));
    ...
    case SEARCH_PRODUCT: {
      try {
        yield initRequest();
        // clear search data
        yield put(clearProductSearchState());

  ...
  switch (type) {
    ...
    case ADD_ORDER: {
      try {
        ...
        yield handleAction(undefined, 'Order successfully added', 'success');
        yield put(setLoading(false));
      } catch (e) {
        yield handleError(e);
        yield handleAction(
          undefined,
          `Order failed to add: ${e?.message}`,
          'error',
        );
      }
      break;
      ...
      try {
        ...
        if (imageCollection.length > 1) {
          ...
          const imageKeys = yield all(
            newUploads.map(() => firebase.generateOrderKey),
          );
          const imageUrls = yield all(
            newUploads.map((img, i) =>
              firebase.storeImage(imageKeys[i](), 'orders', img.file),
            ),
          );
    ...
    case SEARCH_ORDER: {
      try {
        yield initRequest();
        // clear search data
        yield put(clearOrderSearchState());
        ...
        if (result.orders.length === 0) {
          ...
          yield put(
            searchOrderSuccess({
              ...
              lastKey: result.lastKey
                ? result.lastKey
                : state.orders.searchedOrders.lastRefKey,
              total: result.total
                ? result.total
                : state.orders.searchedOrders.total,
            }),
          );
          yield put(setRequestStatus(''));
        }
